...
class LayoutHelper extends AppHelper
{
    ...
    public function js()
    {
        ...
        $cms['cfg'] = array(
			...
			'maximum_job_title_length' => Configure::read('job.maximum_title_length'),
		    'maximum_job_description_length' => Configure::read('job.maximum_description_length'),
        );
    ...
    public function menu($menuAlias, $options = array())
    {
        $_options = array(
            'tag' => 'ul',
            'tagAttributes' => array(
                'class' => 'unstyled clearfix span15 mob-no-mar'
            ) ,
            'selected' => 'selected',
            'dropdown' => false,
	...
	function adminMenus($menus, $options = array())
    {
		...
		$out = '<div class="navbar no-mar"><div class="navbar-inner no-round clearfix no-pad"><a data-target=".nav-collapse" data-toggle="collapse" class="btn btn-navbar ver-mspace"> <i class="icon-align-justify icon-24 blackc"></i></a><div class="nav-collapse">';
        $out.= '<ul class="nav admin-menu row container-fluid">';
        $icon_class = '';
		$intHeader = 0;
		$intSubMenu = 1;
        ...
        foreach($sorted as $menu) {
			...
			if (!empty($menu['icon-class']) && $this->request->params['controller'] == 'settings' && in_array($menu['icon-class'], array('bullhorn', 'group', 'cogs')) && !empty($this->request->params['pass']['0'])) {
				if (!empty($menu['icon-class']) && $this->request->params['pass']['0'] == '85' && $menu['icon-class'] == 'bullhorn') {
					$active_class = 'active';
				...
				if (!empty($menu['allowedActions']) || !empty($menu['notAllowedActions'])) {
					...
					if (!empty($menu['icon-class']) && empty($icon_class) && !empty($active_class)) {
			...
			//$active_class = (in_array($this->request->params['controller'], $controller_arr)) ? ' active' : null;
            $class = '';
            if(!empty($menu['icon-class']) && $menu['icon-class'] == 'admin-payment' && ($this->request->params['controller'] == 'pledges' || $this->request->params['controller'] == 'donates' || $this->request->params['controller'] == 'lends' || $this->request->params['controller'] == 'equities') && $this->request->params['action'] == 'admin_funds') {
                $payment_active = 'active';
                $project_active = '';
                $out.= '<li class="pull-left dc sep dropdown pr ' . $payment_active .  $class .' ">';
            } else if(!empty($menu['icon-class']) && $menu['icon-class'] == 'file' && ($this->request->params['controller'] == 'pledges' || $this->request->params['controller'] == 'donates' || $this->request->params['controller'] == 'lends' || $this->request->params['controller'] == 'equities') && $this->request->params['action'] == 'admin_index') {
                $project_active = 'active';
                $payment_active = '';
                $out.= '<li class="pull-left dc sep dropdown pr ' . $project_active .  $class .' ">';
            } else if(!empty($menu['icon-class']) && $menu['icon-class'] == 'cogs' && $this->request->params['controller'] == 'settings' && isset($this->request->params['pass'][0]) && $this->request->params['pass'][0] == '21') {
                $project_active = '';
                $payment_active = 'active';
                $out.= '<li class="pull-left dc sep dropdown pr ' . $payment_active .  $class .' ">';
            } else  {
                   if($menu['title'] == 'Payments' && ($this->request->params['controller'] == 'pledges' || $this->request->params['controller'] == 'donates' || $this->request->params['controller'] == 'lends' || $this->request->params['controller'] == 'equities' || $this->request->params['controller'] == 'settings')) {
					...
					if(!empty($menu['icon-class']) && $this->request->params['controller'] == 'job_types' && $menu['icon-class'] == 'random')
					{
						$active_class = 'active';
					}

                $out.= '<li class="dc dropdown mob-clr span3 tab-clr ' . $active_class .  $class .' ">';
            }
			...
			if(!empty($menu['icon-class'])){
				$out.= '<span class="show dc no-mar text-42 whitec"><i class="no-mar top-space over-hide show icon-'.$menu['icon-class'].'"></i></span>';
			}
			$out.= '<span class="show text-13 whitec">';
            $out.= $menu['title'];
            $out.= '</span></a>';
           	 if (!empty($menu['icon-class']) && $menu['icon-class'] == 'random') {
				...
				$out.= '<span class="hor-space sfont textn no-mar"><i class="icon-exclamation-sign hor-smspace"></i>';
                ...
                if (!empty($menu['children'])) {
                    if (!empty($menu['column']) && $menu['column'] > 1) {
						...
						if (!empty($menu['icon-class']) && $menu['icon-class'] == 'cogs') {
							$out.= '<li class="hor-mspace">';
							$out.= '<div class=" bot-space">';
							$out.= $this->Html->link('<span class="pull-right blackc smspace"><i class="icon-cog"></i>'.__l('Settings Overview').'</span>',array('controller' => 'settings', 'action'=>'index'),array('class' => 'cur', 'escape'=>false,'title' => __l('Settings Overview')));
							$out.= '</div></li>';
						}
						if (!empty($menu['icon-class']) && $menu['icon-class'] == 'admin-masters') {
							$out.= '<li class="hor-mspace">';
						...
						if (empty($menu['is_hide_title'])) {
							$out.= '<li class="hor-mspace"><h6><span class = "span">' . $menu['title'] . '</span></h6></li>';
						}
						$out.= '<li class="span14 no-mar"><div class="row"><div class="span7">';
                        for ($i = 1; $i <= $menu['column']; $i++) {
                            if ($i == 1) {
								...
								if($menu['icon-class'] == 'align-justify') { $to = $to-4; } else { $to = $to-2;  }
                            } else {
								$out.= '</div><div class="span7">';
                                $from = $to+1;
                                $to = count($menu['children']);
                                $li_class = 'admin-sub-links-right';
                            }
							$out.= '<ul class="unstyled hor-mspace clearfix span">';
                            $out.= $this->_getLinks($menu, $from, $to);
                            $out.= '</ul>';
                        }
                        $out.= '</div></div></li></ul>';
                    } else {
                        $out.= '<ul class="dropdown-menu dl span5">';
        ...
        foreach($menu['children'] as $child) {
            if ((empty($from) && empty($to)) || ($i >= $from && $i <= $to)) {
				...
				if(!empty($child['url']['controller']) && $child['url']['controller'] == 'payment_gateways') {
					$icon.= '<i class="icon-cog text-16 "></i>';
				}
                if (!empty($child['url'])) {
