	...
	<div class="accordion-admin-panel" id="js-admin-panel">
		<div class="clearfix js-admin-panel-head admin-panel-block">
			<div class="admin-panel-inner span4 pa accordion-heading no-mar no-bor clearfix box-head admin-panel-menu">
				<a data-toggle="collapse" data-parent="#accordion-admin-panel" href="#adminPanel" class="btn js-show-panel accordion-toggle js-toggle-icon blackc no-under clearfix span4"><i class="pull-right caret"></i><i class="icon-user"></i> <?php echo __l('Admin Panel'); ?></a>
			</div>
			<div class="accordion-body no-round no-bor collapse" id="adminPanel">
				<div id="ajax-tab-container-admin" class="accordion-inner thumbnail clearfix no-bor tab-container admin-panel-inner-block">
					<ul class="nav no-float nav-tabs tabs">
						<li class="tab"><?php echo $this->Html->link(__l('Actions'), '#admin-actions',array('title'=>__l('Actions'), 'class' => ' js-no-pjax', 'data-toggle'=>'tab', 'rel' => 'address:/admin_actions')); ?></li>
						<li class="tab"><em></em><?php echo $this->Html->link(sprintf(__l('User Logins (%s)'), $this->Html->cInt($user['User']['user_login_count'])), array('controller' => 'user_logins', 'action' => 'index', 'user_id' => $user['User']['id'], 'view_type' => 'user_view', 'admin' => true), array('data-target'=>'#admin-user-logins', 'class' => 'js-no-pjax','escape' => false)); ?></li>
					</ul>
						...
						<div class="span20 tab-pane fade in active" id="admin-actions" style="display: block;">
							<ul class="unstyled clearfix">
							  ...
							  <?php endif; ?>
								...
								<li class="pull-left dc mspace">
								  <?php echo $this->Html->link('<i class="icon-remove"></i> '.__l('Delete'), Router::url(array('users', 'action'=>'admin_delete', $user['User']['id'],'admin'=> true),true).'?r='.$this->request->url, array('class' => 'btn blackc js-confirm js-no-pjax', 'escape'=>false,'title' => __l('Delete')));?>
								</li>
								  <?php } ?>
								...
								<li class="pull-left dc mspace">
								  <?php echo $this->Html->link('<i class="icon-lock"></i> '.__l('Change password'), array('controller' => 'users', 'action'=>'admin_change_password', $user['User']['id'], 'admin'=> true), array('class' => 'btn blackc js-no-pjax', 'escape'=>false,'title' => __l('Change password')));?>
								</li>
							  <?php endif; ?>
							</ul>
						</div>
