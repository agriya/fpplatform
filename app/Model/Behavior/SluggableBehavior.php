 ...
 */
/* SVN FILE: $Id: sluggable.php 271 2009-10-16 13:36:57Z rajesh_059at09 $ */
/**
...
class SluggableBehavior extends ModelBehavior
{
    ...
    function beforeSave(Model $Model)
    {
        ...
        if (!is_array($this->__settings[$Model->alias]['label'])) {
            $this->__settings[$Model->alias]['label'] = array(
                $this->__settings[$Model->alias]['label']
            );
        }
        ...
        if ((!$this->__settings[$Model->alias]['real'] || $Model->hasField($this->__settings[$Model->alias]['slug'])) && ($this->__settings[$Model->alias]['overwrite'] || empty($Model->id))) {
            ...
            if (!empty($label)) {
                ...
                $conditions = array(
                    $Model->alias . '.' . $this->__settings[$Model->alias]['slug'] . ' LIKE' => $slug . '%'
                );
                if (!empty($Model->id)) {
                ...
                // fixed: if a slug already created like this "test-by-me" for tag "test by me". After this anybody add tag like "test" it created slug like this "test-1"
                if (!empty($sameUrls) && in_array($slug, $sameUrls)) {
                    $begginingSlug = $slug;
                    $index = 1;
    ...
    function __slug($string, $settings)
    {
        if (!empty($settings['translation']) && is_array($settings['translation'])) {
        ...
        } else if (!empty($settings['translation']) && is_string($settings['translation']) && in_array(strtolower($settings['translation']) , array(
            'utf-8',
            'iso-8859-1'
        ))) {
            ...
            return $this->__slug($string, array_merge($settings, array(
                'translation' => $translations[$settings['translation']]
            )));
        }
        // Was	$string = strtolower($string);
        // Was	$string = preg_replace('/[^a-z0-9_]/i', $settings['separator'], $string);
        // Was	$string = preg_replace('/' . preg_quote($settings['separator']) . '[' . preg_quote($settings['separator']) . ']*/', $settings['separator'], $string);
        //abuthakir_060at09: strtolower replaced with mb_strtolower to support unicode characters
        $string = mb_strtolower($string, 'UTF-8');
        $string = preg_replace('/[\.\!\$\*\(\)\=\{\}\[\]\;\|\%\#\/\?\:\s\&\+\`\^\'\"\,\>\<\@]/i', $settings['separator'], $string);
