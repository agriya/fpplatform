...
class OrderedBehavior extends ModelBehavior
{
    ...
    public function afterDelete(Model $Model)
    {
        if ($Model->data) {
            ...
            $action = array(
                $this->settings[$Model->alias]['field'] => $this->settings[$Model->alias]['field'] . ' - 1'
            );
            $conditions = array(
    ...
    public function moveUp(Model $Model, $id = null, $number = 1)
    {
        ...
        if (is_numeric($number)) {
            if ($number == 1) { // move 1 space
                ...
                $data[0] = $Model->data;
                $data[1] = $previous;
                return $Model->saveAll($data, array(
                    'validate' => false
                ));
                ...
                $Model->updateAll(array(
                    $this->settings[$Model->alias]['field'] => $Model->alias . '.' . $this->settings[$Model->alias]['field'] . ' + 1'
                ) , $conditions);
    ...
    public function resetWeights(Model $Model)
    {
        if ($this->settings[$Model->alias]['foreign_key']) {
            ...
            foreach($foreignKeys as $fk) {
                $all = $Model->find('all', array(
                    'conditions' => array(
                        $this->settings[$Model->alias]['foreign_key'] => $fk
                    ) ,
                    'fields' => array(
                        $Model->displayField,
                        $Model->primaryKey,
    ...
    {
        $conditions = array(
            $this->settings[$Model->alias]['field'] => $weight
        );
        $fields = array(
            $Model->primaryKey,
            $this->settings[$Model->alias]['field']
