...
class TaggableBehavior extends ModelBehavior
{
    ...
    function findForeignIDs(Model $model) 
    {
        foreach($model->hasAndBelongsToMany as $assocKey => $assocData) {
			if ($assocData['className'] != 'Request') {
				$classname = explode( '.', $assocData['className']);
				$assocData['className']=trim($classname[1]);
    ...
    function beforeSave(Model $model) 
    {
        foreach($model->hasAndBelongsToMany as $assocKey => $assocData) {
			...
			if ($assocData['className'] != 'Request') {
				// If Jobs.JobTag shows error so using explode we remove the jobs
				$classname = explode( '.', $assocData['className']);
				$assocData['className']=trim($classname[1]);
				$assocModel = $model->{$assocData['className']};
				$field = Inflector::underscore($model->name) . '_count';
				if (isset($model->data[$model->name]['tag']) && $assocModel->hasField($field , true)) {
					if (!empty($model->id)) {
            ...
            foreach($tagNames as $tagName) {
                ...
                if (!empty($tagName)) {
                    ...
                    if (!empty($tags)) {
						...
						$data[$assocModel->name]['slug'] = Inflector::slug($tagName, $replacement = '-');
                        $assocModel->create();
