 ...
 */
class CmsEventManager extends CakeEventManager
{
    /**
    ...
    /**
     * Returns the globally available instance of a CmsEventManager
     * @return CmsEventManager the global event manager
     */
    public static function instance($manager = null)
    {
        if (empty(self::$_generalManager)) {
            return parent::instance(new CmsEventManager());
        }
            ...
            if (!empty($eventHandlers) && is_array($eventHandlers)) {
                foreach($eventHandlers as $eventHandler => $eventOptions) {
                    ...
                    if (class_exists($class)) {
                        ...
                        logError(sprintf(__l('EventHandler %s not found in plugin %s'), $class, $plugin));
                    }
                }
            }
        }
