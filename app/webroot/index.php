...
function updateViews($main_table_name,$main_field_name,$view_table_name,$main_id)
{
	...
	if($database->default['datasource']=='Database/Mysql') {
		...
		if(stripos($view_table_name, 'project_views') !== false){
			$project_view_type_id = 1; // normal view type id
			$view_count_column = $main_field_name . '_view_count';
			if(stripos($_SERVER["REQUEST_URI"], '/widget') !== false){
				$project_view_type_id = 2; // embedded view type id
				$view_count_column = 'embed_view_count';
			}
			mysql_query('INSERT INTO `' . $view_table_name . '` (`created`, `modified`, `user_id`, `' . $main_field_name . '_id`, `ip_id`, `project_view_type_id`) VALUES (now(), now(), ' . $user_id . ', ' . $main_id . ', ' . $ip_id . ', '. $project_view_type_id . ')');
			$view_result = mysql_query('SELECT COUNT(*) as count FROM ' . $view_table_name . ' WHERE project_view_type_id = '.$project_view_type_id.' AND ' . $main_field_name . '_id = ' . $main_id);
			$view_row = mysql_fetch_assoc($view_result);
			mysql_query('UPDATE `' . $main_table_name . '` SET '.$view_count_column.' = "' . $view_row['count'] . '" WHERE id = ' . $main_id);
		} else {
		...
		if (stripos($view_table_name, 'project_views') !== false) {
			...
			pg_query($db, 'INSERT INTO ' . $view_table_name . ' ("created", "modified", "user_id", "' . $main_field_name . '_id", "ip_id", "project_view_type_id") VALUES (now(), now(), ' . $user_id . ', ' . $main_id . ', ' . $ip_id . ', '. $project_view_type_id . ')');
			$view_result = pg_query($db, 'SELECT COUNT(*) as count FROM ' . $view_table_name . ' WHERE project_view_type_id = '.$project_view_type_id.' AND ' . $main_field_name . '_id = ' . $main_id);
			$view_row = pg_fetch_assoc($view_result);
			pg_query($db, 'UPDATE ' . $main_table_name . ' SET ' . $view_count_column . ' = ' . $view_row['count'] . ' WHERE id = ' . $main_id);
		} else {
