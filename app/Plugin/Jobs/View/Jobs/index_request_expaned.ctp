...
if (!empty($jobs)):
?>
<ol class="unstyled well no-pad no-bor no-round no-shad discussion-list clearfix">
<?php
$i = 0;
foreach ($jobs as $job):
	$class = null;
...
?>
	<li class="ver-space sep-top no-mar clearfix <?php echo $purchase; ?>">
		...
		<?php } ?>
			...
			<div class="clearfix">
				...
				<?php if(isPluginEnabled('HighPerformance')&& (Configure::read('HtmlCache.is_htmlcache_enabled') || Configure::read('cloudflare.is_cloudflare_enabled')))  { ?>
				 ...
				 <?php echo $this->Html->link($this->Html->cText($job['Job']['title'], false), array('controller' => 'jobs', 'action' => 'view', $job['Job']['slug']), array('title' => $this->Html->cText($job['Job']['title'],false),'escape' => false, 'class' => 'span12 span13-sm no-mar htruncate graydarkerc'));?></h3>
				 </div>
                 <p>
			...
			<div class="top-space span6 span6-sm no-mar pull-right">
			<?php if(isPluginEnabled('HighPerformance')&& (Configure::read('HtmlCache.is_htmlcache_enabled') || Configure::read('cloudflare.is_cloudflare_enabled')))  { ?>
				<div class="bl-buy-<?php echo $job['Job']['id'];?> hide  ">
					<?php echo $this->Html->link(__l('Buy Now'), array('controller' => 'users', 'action' => 'login'), array('class'=>'btn btn-success pull-right hor-mspace textb','title' => __l('Buy Now')));?>
				</div>
				<div class="al-buy-instr-<?php echo $job['Job']['id'];?> hide  ">
					<?php echo $this->Html->link(__l('Buy Now'), array('controller'=>'job_orders','action'=>'add','job' => $job['Job']['id']), array('title' => __l('Buy Now'), 'class' => 'btn btn-success pull-right hor-mspace textb','escape' => false));?>
				</div>
				<div class="al-buy-<?php echo $job['Job']['id'];?> hide ">
					<?php echo $this->Html->link(__l('Buy Now'), array('controller'=>'payments','action'=>'order',$job['Job']['id']), array('title' => __l('Buy Now'), 'class' => 'btn btn-success pull-right hor-mspace textb'));?>
				</div>
				<div class="al-edit-<?php echo $job['Job']['id'];?> hide ">
					<?php echo $this->Html->link(__l('Edit'), array('controller'=>'jobs','action'=>'edit',$job['Job']['id']), array('title' => __l('Edit'), 'class' => 'btn btn-success pull-right hor-mspace textb'));?>
				</div>
			<?php } else { ?>
			...
			<div class=" tab-clr clearfix">
				<div class="pull-right mob-inline">
					<p class="pull-left text-30"><?php echo $this->Html->siteCurrencyFormat($job['Job']['amount']);?></p>
				</div>
			</div>
		</div>
	</li>
...
?></ol>
<?php
	elseif(isset($this->request->params['named']['source'])):
?>
	<div class="grayc notice text-16 dc"><?php echo __l('No records available');?></div>
<?php
	endif;
		if (!empty($jobs) && count($jobs)>10) {?>
