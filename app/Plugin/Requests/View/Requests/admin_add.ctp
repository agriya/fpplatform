...
<ul class="breadcrumb no-round top-mspace ver-space bot-mspace">
	  ...
	  <li class="active graydarkerc"><?php echo __l('Add '). requestAlternateName(ConstRequestAlternateName::Singular, ConstRequestAlternateName::FirstLeterCaps);?></li>      
	</ul>
	...
	<?php echo $this->Form->create('Request', array('class' => 'normal form-horizontal  js-add-map'));?>		
            ...
            <?php 	
				...
				if(count($amount) > 1):
					...
					echo $this->Form->input('amount', array($this->Html->siteCurrencyFormat(Configure::read('job.price')), 'label' => __l('Amount').' ('.Configure::read('site.currency').')'));									
           ...
           <?php 
			...
			<?php
				echo $this->Form->submit((__l('Add')), array('name' => "data[Request][continue]", 'class' => 'btn btn-large btn-warning textb text-20 start' ));
			?>
			</div>
...
<?php }else{?>
		...
		<div class="js-related-jobs-load">
			<h3> <?php echo sprintf(__l('Related %s'), requestAlternateName(ConstRequestAlternateName::Plural,ConstRequestAlternateName::FirstLeterCaps)); ?> </h3>
			<?php
