...
class GoogleAnalyticsController extends AppController
{
	...
	public function admin_form_bounce_chart()
    {
		...
		$type_name = 'Job';
		$job_add_form_bounce_arr = array();
		$job_fund_form_bounce_arr = array();
		// job add form bounce query
		$stats = $this->_getStats($condition, 'ga:totalEvents', array('dimensions' => 'ga:eventLabel', 'sort' => 'ga:eventLabel', 'filters' => 'ga:eventCategory=~Job;ga:eventAction=~'. $type_name .'Posted;ga:eventLabel=~Step '));
		$job_add_steps_arr = array();
		$job_add_events_arr = array();
		if(!empty($stats->rows)){
		foreach($stats->rows as $stat) {
			$job_add_steps_arr[] = "'" . $stat[0] . "'";
			$job_add_events_arr[] = $stat[1];
		}
		}
		$job_add_form_bounce_arr[] = '["Form", ' . implode(', ', $job_add_steps_arr) . ']';
		$job_add_form_bounce_arr[] = '["'.$type_name.' Job Post", ' . implode(', ', $job_add_events_arr) . ']';			
		$this->set($type_name . 'job_add_form_bounces', implode(',', $job_add_form_bounce_arr));
		// job order form bounce query
		$stats = $this->_getStats($condition, 'ga:totalEvents', array('dimensions' => 'ga:eventLabel', 'sort' => 'ga:eventLabel', 'filters' => 'ga:eventCategory=~JobOrder;ga:eventAction=~Order;ga:eventLabel=~Step '));
		$job_fund_events_arr = array();
		$job_fund_steps_arr = array();
		if (!empty($stats->rows)) {
			foreach($stats->rows as $stat) {
				$job_fund_steps_arr[] = "'" . $stat[0] . "'";
				$job_fund_events_arr[] = $stat[1];
			}
			if (!empty($job_fund_events_arr)) {
				$job_fund_form_bounce_arr[] = '["Form", ' . implode(', ', $job_fund_steps_arr) . ']';
				$job_fund_form_bounce_arr[] = '["Property Fund", ' . implode(', ', $job_fund_events_arr) . ']';
			}
		}
		$this->set($type_name . 'job_fund_form_bounces', implode(',', $job_fund_form_bounce_arr));
	}
