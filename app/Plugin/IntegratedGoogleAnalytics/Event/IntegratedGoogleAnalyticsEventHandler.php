    ...
    public function pushScript($event)
    {
		...
		if (!empty($_SESSION['_trackEvent'])) {
			foreach($_SESSION['_trackEvent'] as $trackEvent) {
				...
				$return .= "_gaq.push(['_trackEvent', '" . $trackEvent['_trackEvent']['category'] . "', '" . $trackEvent['_trackEvent']['action'] . "', '" . $trackEvent['_trackEvent']['label'] . "', " . $trackEvent['_trackEvent']['value'] . "]);";
				$return .= "</script>";
			}
		...
		if (!empty($_SESSION['_trackEcommerce'])) {
			foreach($_SESSION['_trackEcommerce'] as $trackEcommerce) {
				...
				if (!empty($trackEcommerce['_addTrans'])) {
					$return .= "_gaq.push(['_addTrans', '" . $trackEcommerce['_addTrans']['order_id'] . "', '" . $trackEcommerce['_addTrans']['name'] . "', ' " . $trackEcommerce['_addTrans']['total'] . "', '', '', '', '', '']);";
				}
