...
class UserCashWithdrawalsController extends AppController
{
    ...
    {
        if (empty($this->is_wallet_enabled)) {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->UserCashWithdrawal->validates() &$this->UserCashWithdrawal->validationErrors['amount'] = $this->UserCashWithdrawal->_checkAmount($this->request->data['UserCashWithdrawal']['amount'])) {
                ...
                if ($this->UserCashWithdrawal->save($this->request->data)) {
                    $this->UserCashWithdrawal->User->Transaction->log_data($this->UserCashWithdrawal->getLastInsertId() , 'Withdrawals.UserCashWithdrawal', ConstPaymentGateways::ManualPay, ConstTransactionTypes::CashWithdrawalRequest);
    ...
    public function admin_index()
    {
        ...
        if (isset($this->request->params['named']['stat']) && $this->request->params['named']['stat'] == 'day') {
            $conditions['UserCashWithdrawal.created <= '] = date('Y-m-d', strtotime('now')) . ' 00:00:00';
            $this->pageTitle.= __l(' - Requested today');
        }
        if (isset($this->request->params['named']['stat']) && $this->request->params['named']['stat'] == 'week') {
            $conditions['UserCashWithdrawal.created >= '] = date('Y-m-d', strtotime('now -7 days'));
            $this->pageTitle.= __l(' - Requested in this week');
        }
        if (isset($this->request->params['named']['stat']) && $this->request->params['named']['stat'] == 'month') {
            $conditions['UserCashWithdrawal.created >= '] = date('Y-m-d', strtotime('now -30 days'));
        ...
        if (!empty($this->request->data['UserCashWithdrawal']['account_id']) && $this->request->data['UserCashWithdrawal']['account_id'] != 'all') {
            $userCashWithdrawals = $this->UserCashWithdrawal->find('all', array(
                'conditions' => array(
                    'UserCashWithdrawal.payment_gateway_id' => $this->request->data['UserCashWithdrawal']['account_id'],
                ) ,
                'contain' => array(
                    'User' => array(
                ...
                if ($actionid == ConstWithdrawalStatus::Pending) {
                    ...
                    foreach($canceled_withdraw_requests as $canceled_withdraw_request) {
                        ...
                        if (!empty($canceled_withdraw_request)) {
                            $this->UserCashWithdrawal->User->Transaction->log_data($canceled_withdraw_request['UserCashWithdrawal']['id'], 'Withdrawals.UserCashWithdrawal', ConstTransactionTypes::CashWithdrawalRequestRejected);
                        }
