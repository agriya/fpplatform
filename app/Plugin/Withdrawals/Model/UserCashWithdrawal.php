...
class UserCashWithdrawal extends AppModel
{
    ...
    function _checkAmount($amount)
    {
        ...
        if (($amount < Configure::read('user.minimum_withdrawal_amount')) || ($amount > Configure::read('user.maximum_withdrawal_amount'))) {
            return sprintf(__l('Given amount should lies from  %s%s to %s%s') , Configure::read('site.currency') , Configure::read('user.minimum_withdrawal_amount') , Configure::read('site.currency') , Configure::read('user.maximum_withdrawal_amount'));
        }
        return true;
    }
    ...
    {
        $transaction_id = $this->User->Transaction->log_data($userCashWithdrawal['UserCashWithdrawal']['id'], 'Withdrawals.UserCashWithdrawal', ConstTransactionTypes::CashWithdrawalRequestPaid);
    ...
    {
        $transaction_id = $this->User->Transaction->log_data($userCashWithdrawal['UserCashWithdrawal']['id'], 'Withdrawals.UserCashWithdrawal', $userCashWithdrawal['UserCashWithdrawal']['payment_gateway_id'], ConstTransactionTypes::CashWithdrawalRequestFailed);
        $this->User->updateAll(array(
        ...
        $this->
        {
            ...
            foreach($userCashWithdrawalIds as $userCashWithdrawalId) {
                ...
                if (!empty($userCashWithdrawalId) && !empty($cash_withdraw)) {
                    $transaction_id = $this->User->Transaction->log_data($cash_withdraw['UserCashWithdrawal']['id'], 'Withdrawals.UserCashWithdrawal', ConstPaymentGateways::SudoPay, ConstTransactionTypes::CashWithdrawalRequestApproved);
