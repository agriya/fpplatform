...
class SearchableBehavior extends ModelBehavior
{
    ...
    public function fill() 
    {
        ...
        // Create index
        $u = $this->execute($Model, 'PUT', '', array(
            'fullIndex' => true,
        ));
        $d = $this->execute($Model, 'DELETE', '');
        $o = $this->execute($Model, 'POST', '_refresh', array(
            'fullIndex' => true,
        ));
        // Get records
        $offset = 0;
        ...
        // Index needs a moment to be updated
        $this->execute($Model, 'POST', '_refresh', array(
            'fullIndex' => true,
        ));
        return $count;
    }
    ...
    protected function _fillChunk($Model, $offset, $limit, $id = null) 
    {
        ...
        foreach($results as $result) {
            ...
            if (empty($doc[$primKeyPath])) {
                return $this->err($Model, 'I need at least primary key: %s->%s inside the index data. Please include in the index_find_params', $Model->alias, $Model->primaryKey);
            }
            $meta = array(
                '_index' => $index_name,
                '_type' => $type,
        ...
        if ($docCount == 1) {
            ...
            $res = $this->execute($Model, 'PUT', '_bulk', $commands, array(
                'prefix' => '',
            ));
        }
        if (is_string($res)) {
            return $this->err($Model, 'Unable to add items. %s', $res);
        } else {
    ...
    public function execute($Model, $method, $path, $payload = array() , $options = array()) 
    {
        ...
        $path = $prefix . $path;
        $uri = sprintf('http://%s:%s/%s', $this->opt($Model, 'host') , $this->opt($Model, 'port') , $path);
    ...
    protected function _filter_fields($Model, $val) 
    {
        if ($val === '_all' || empty($val)) {
            if (!$this->opt($Model, 'fields_excludes')) {
                ...
                $val = $this->_allFields($Model, $this->opt($Model, 'fields_excludes'));
            }
        }
        return $val;
    }
    ...
    protected function _filter_highlight($Model, $val) 
    {
        ...
        if (($params = @$val['fields']['_all'])) {
            ...
            $fields = $this->_allFields($Model, $this->opt($Model, 'highlight_excludes'));
    ...
    {
        $this->settings[$Model->alias] = array_merge($this->_default, $settings);
    ...
    public function opt() 
    {
        ...
        if ($count > 1) {
            ...
            if (!array_key_exists($key, $this->settings[$Model->alias])) {
                return $this->err($Model, 'Option %s was not set', $key);
            }
            ...
            return $this->err($Model, 'Found remaining arguments: %s Opt needs more arguments (1 for Model; 1 more for getting, 2 more for setting)', $args);
        }
    }
}
