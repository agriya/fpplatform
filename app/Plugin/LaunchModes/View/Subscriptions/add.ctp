<?php if (Configure::read('site.launch_mode') == 'Private Beta') { ?>
  <?php if (!$success_msg): ?>
    <?php
      echo $this->Form->create('Subscription', array('class' => "ver-space dl clearfix no-mar js-ajax-form"));
      if(!empty($error_message)) {
        $label = __l('You may request for new invitation code');
      } else {
    ...
    <div class="submit btn-align span"><?php echo $this->Form->submit(__l('Request Invite'), array('class'=>'btn btn-primary')); ?></div>
...
<?php } else { ?>
  <?php if (!$success_msg): ?>
    <div class="clearfix">
      ...
      <div class="submit btn-align span"><?php echo $this->Form->submit(__l('Notify Me'), array('class' => 'btn btn-primary')); ?></div>
      <?php echo $this->Form->end(); ?>
    </div>
    <div class="dl bot-space hor-mspace"><?php echo __l('By submitting this email, I am authorizing site to send me emails until I unsubscribe.'); ?>
    <?php echo $this->Html->link(__l('Privacy Policy'), array('controller' => 'pages', 'action' => 'view', 'privacy-policy', 'admin' => false), array('title' => __l('Privacy Policy'), 'class'=>'js-no-pjax', 'data-toggle' => 'modal', 'data-target' => '#js-ajax-modal'));?>
    </div>
