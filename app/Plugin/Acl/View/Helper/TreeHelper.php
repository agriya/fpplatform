 ...
 */
/* SVN FILE: $Id: tree.php 6408 2010-02-05 11:09:52Z preethi_083at09 $ */
/**
 * Tree Helper.
 *
...
class TreeHelper extends AppHelper
{
     ...
     * @access private
     */
    public $__typeAttributesNext = array();
    /**
    ...
    public function generate($data, $settings = array()) 
    {
        ...
        foreach($data as $i => $result) {
            ...
            if ($element) {
                ...
                list($content) = array_map($callback, array(
                    $elementData
                ));
            } else {
            ...
            /* Prefix */
            if ($__addType) {
                $typeAttributes = $this->__attributes($type, array(
                    'data' => $elementData
                ));
                $return.= "\r\n" . str_repeat("\t", count($stack)) . '<' . $type . $typeAttributes . '>';
            }
            $itemAttributes = $this->__attributes($itemType, $elementData);
            $itemtype_class = ' class="open"';
            if (empty($elementData['hasChildren'])) {
                $itemtype_class = ' class="nosubmenu"';
            }
            $return.= "\r\n" . str_repeat("\t", count($stack) +1) . '<' . $itemType . $itemtype_class . $itemAttributes . '>';
            $return.= '<span>' . $content . '<span class="tree-actions">';
            if ($this->__settings['show_add_role']) {
                $return.= ' ';
                $return.= $this->Html->link(sprintf(__l('Add %s') , __l('Role')) , array(
                    'controller' => 'roles',
                    'action' => 'add',
                    $result[$model]['id']
                ) , array(
                    'class' => 'add',
                    'title' => sprintf(__l('Add %s') , __l('Role'))
                ));
            }
            if ($this->__settings['show_edit_role']) {
                $return.= $this->Html->link(sprintf(__l('Edit %s') , __l('Role')) , array(
                    'controller' => 'roles',
                    'action' => 'edit',
                    $result[$model]['id']
                ) , array(
                    'class' => 'edit',
                    'title' => sprintf(__l('Edit %s') , __l('Role'))
                ));
            }
            if ($this->__settings['show_delete_role']) {
                $return.= $this->Html->link(sprintf(__l('Delete %s') , __l('Role')) , array(
                    'controller' => 'roles',
                    'action' => 'delete',
                    $result[$model]['id']
                ) , array(
                    'class' => 'delete js-confirm',
                    'title' => sprintf(__l('Delete %s') , __l('Role'))
                ));
            }
            if ($this->__settings['show_move_role']) {
                $return.= $this->Html->link(sprintf(__l('Move %s') , __l('Role')) , '#', array(
                    'class' => 'move',
                    'title' => sprintf(__l('Move %s') , __l('Role'))
                ));
            }
            $return.= '</span></span>';
            /* Suffix */
    ...
    /**
     * addItemAttribute public function
     *
    ...
    /**
     * addTypeAttribute public function
     *
     ...
     * @return void
     */
    public function __attributes($rType, $elementData = array() , $clear = true) 
    {
        extract($this->__settings);
        if ($rType == $type) {
