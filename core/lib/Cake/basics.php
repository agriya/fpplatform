 ...
 */
	...
	function jobAlternateName($verb = null, $type = null)
    {
        $lang_code = !empty($_COOKIE['CakeCookie']['user_language']) ? $_COOKIE['CakeCookie']['user_language'] : Configure::read('site.language');
        $alternate_name = ($lang_code == 'en') ? Configure::read('job.job_alternate_name') : 'job';
        if (empty($alternate_name)) {
            $alternate_name = 'job';
        }
        if ($verb == ConstJobAlternateName::Plural) {
            $alternate_name = Inflector::pluralize($alternate_name);
        } elseif ($verb == ConstJobAlternateName::Singular) {
            $alternate_name = Inflector::singularize($alternate_name);
        }
        switch ($type) {
            case ConstJobAlternateName::FirstLeterCaps:
	...
	function requestAlternateName($verb = null, $type = null)
    {
        $lang_code = !empty($_COOKIE['CakeCookie']['user_language']) ? $_COOKIE['CakeCookie']['user_language'] : Configure::read('site.language');
        $alternate_name = ($lang_code == 'en') ? Configure::read('request.request_alternate_name') : 'request';
        if (empty($alternate_name)) {
            $alternate_name = 'request';
        }
        if ($verb == ConstRequestAlternateName::Plural) {
            $alternate_name = Inflector::pluralize($alternate_name);
        } elseif ($verb == ConstRequestAlternateName::Singular) {
            $alternate_name = Inflector::singularize($alternate_name);
        }
        switch ($type) {
            case ConstRequestAlternateName::FirstLeterCaps:
                $alternate_name = ucfirst($alternate_name);
                break;
