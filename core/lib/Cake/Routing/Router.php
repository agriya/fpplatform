...
//<--
// Siva // plugin name append in url
		if (preg_match('/\/install\//', $url, $match)) {
			$out['plugin'] = 'install';
		}
//<--
//<--
// Siva // plugin name append
		$plugins = Configure::read('plugins');
		if (!empty($plugins)) {
			foreach($plugins as $pluginName => $value) {
				if (Configure::read('plugins.' . $pluginName) && preg_match('/' . Configure::read('plugins.' . $pluginName) . '/', '/' . $out['controller'] . '/', $match)) {
...
//<--
// rajesh_04ag02 // 2008-06-19 // for controllers/view/ -> controller/
// @todo Fails for /video/a:1/b:2
    	if (!empty($out['controller']) && $out['controller'] != 'install' && $out['controller'] == Inflector::singularize($out['controller'])){
            $out['controller'] = Inflector::pluralize($out['controller']);
            if (!empty($out['action']) && $out['action'] != 'index'){
                array_unshift($out['pass'], $out['action']);
            }
            $out['action'] = 'view';
        }
//<--
// Siva // Fix for reverse routing namedArg is set in param wrongly
		$skip = array(
			'bare', 'action', 'controller', 'plugin', 'ext', '?', '#', 'prefix', 'admin', 'pass', 'named', 'url', 'form', 'autoRender', 'return', 'requested'
		);
		$keys = array_values(array_diff(array_keys($out), $skip));
		$count = count($keys);
		for ($i = 0; $i < $count; $i++) {
			$out['named'][$keys[$i]] = $out[$keys[$i]];
			unset($out[$keys[$i]]);
		}
//<--
		if (isset($out['prefix'])) {
			...
			if(!empty($url['controller']) )
			{
				...
				if(in_array($url['controller'], $job_array))
				{
					if($url['controller'] == 'jobs')
						$params['controller'] = $url['controller'] = $job_alternate_name_plural;
					else
					{
						$url_exploded= explode('_',$url['controller']);
						unset($url_exploded[0]);
						$params['controller'] = $url['controller'] = $job_alternate_name_singular.'_' . implode('_',$url_exploded) ;
					}
				}
				if(in_array($url['controller'], $request_array))
				{
					if($url['controller'] == 'requests')
						$params['controller'] = $url['controller'] = $request_alternate_name_plural;
					else
					{
						$url_exploded= explode('_',$url['controller']);
						unset($url_exploded[0]);
						$params['controller'] = $url['controller'] = $request_alternate_name_singular.'_' . implode('_',$url_exploded) ;
					}
				}
			}
// rajesh_04ag02 // 2008-06-19 // for controllers/view/ -> controller/
    		if ($url['action'] == 'view') {
				$view_exception_arr = array(
					'nodes'
				);
                if (!empty($url['controller']) && !in_array($url['controller'], $view_exception_arr)) {
                    $url['controller'] = Inflector::singularize($url['controller']);
                }
            }
// <--
// rajesh_04ag02 // 2009-02-20 // for securing links with random hash
// another fix in dispatcher.php#164
            if (($_hashSecuredActions = Configure::read('site._hashSecuredActions')) && !(empty($url['action'])) && in_array($url['action'], $_hashSecuredActions)) {
				$t_url = array();
				if (empty($url['controller'])) {
					$t_url['controller'] = $params['controller'];
				}
				$t_url = array_merge($t_url, $url);
                unset($t_url['plugin']);
                unset($t_url['admin']);
				// fix for request action
				$skip = array_flip(array(
					'bare', 'action', 'controller', 'plugin', 'ext', '?', '#', 'prefix', 'admin', 'pass', 'named', 'url', 'form', 'autoRender', 'return', 'requested', 'id'
				));
...
// <--
			...
			$match = false;
//<--
// Siva // unset plugin from url array
			$url['plugin'] = false;
//<--
			for ($i = 0, $len = count(self::$routes); $i < $len; $i++) {
				$originalUrl = $url;
				...
				if ($match = self::$routes[$i]->match($url)) {
					...
					$output = str_replace('/view/', '/', $output);
					// remove '/index' from URL
					$output = preg_replace('/\/index$/', '', $output);
					$url = array();
					break;
				}
				$url = $originalUrl;
...
// rajesh_04ag02 // 2008-06-19 // for controllers/view/ -> controller/
// Fix for undefined index error below line
			if (!isset($url['action'])) {
				$url['action'] = null;
			}
//<--
...
//rajesh_04ag02 // 2008-07-04 // coding standard
// 2008-08-17 // allowed /img/ as upload behavior's image controller is not refreshing (redirect() using string based url) // quick fix
// 2008-08-31 // allowed /files/ as upload behavior's image controller is not refreshing (redirect() using string based url) // quick fix
			if ((strcmp($url, '/') !=0) && (strpos($url, '/img/') !== 0) && (strpos($url, '/files/') !== 0)){
                //trigger_error('***dev1framework :: URL should be set in array format (i.e., array(\'controller\' => \'controller\', \'action\' => \'action\') for other than "/". Fix your helper usage.', E_USER_ERROR);
            }
			if (substr($url, 0, 1) === '/') {
				$output = substr($url, 1);
			} else {
