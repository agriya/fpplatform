...
class UploadBehavior extends ModelBehavior
{
    ...
    function _processUpload(Model $model, $data = array() , $direct = false)
    {
        ...
        extract($this->settings[$model->name]);
		$filetype = $model->data[$model->name][$fileField]['type'];
		$filesize = $model->data[$model->name][$fileField]['size'];
		// Get file path
		...
		if(!isset($model->data[$model->name][$fileField]['type']) || empty($model->data[$model->name][$fileField]['type'])) {
			$model->data[$model->name]['mimetype'] = $filetype;
		} else {
			$model->data[$model->name]['mimetype'] = $model->data[$model->name][$fileField]['type'];
		}
		if(!isset($model->data[$model->name][$fileField]['size']) || empty($model->data[$model->name][$fileField]['size'])) {
			$model->data[$model->name]['filesize'] = $filesize;	
		} else {
			$model->data[$model->name]['filesize'] = $model->data[$model->name][$fileField]['size'];
		}
		if(!(Configure::read('s3.is_enabled') && Configure::read('s3.upload_type') == 'normal' && !Configure::read('s3.keep_copy_in_local'))) {
