			...
			if (!empty($object->validate)) {
				foreach ($object->validator() as $validateField => $validateProperties) {
					if ($this->_isRequiredField($validateProperties)) {
						...
						$validateRules[$validateField] = $object->validate[$validateField];
					}
				}
			}
			$this->fieldset[$model]['validates'] = $validates;
			$this->fieldset[$model]['validateRules'] = !empty($validateRules) ? $validateRules : '';
		}
 ...
 */
	public function create($model = null, $options = array()) {
		...
		if ($options['action'] === null && $options['url'] === null) {
			...
			// Alternate name handling code start
			if(!empty($options['url']['controller']))
			{
				...
				if(in_array($options['url']['controller'], $job_array))
				{
					if($options['url']['controller'] == 'jobs')
						$options['url']['controller'] = $job_alternate_name_plural;
					else
					{
						$url_exploded= explode('_', $options['url']['controller']);
						unset($url_exploded[0]);
						$options['url']['controller'] = $job_alternate_name_singular.'_' . implode('_',$url_exploded) ;
					}
				}
				if(in_array($options['url']['controller'], $request_array))
				{
					if($options['url']['controller'] == 'requests')
						$options['url']['controller'] = $request_alternate_name_plural;
					else
					{
						$url_exploded= explode('_', $options['url']['controller']);
						unset($url_exploded[0]);
						$options['url']['controller'] = $request_alternate_name_singular.'_' . implode('_',$url_exploded) ;
					}
				}
			}
			// Alternate name handling end
...
// <--
		...
		switch ($type) {
			...
			case 'checkbox':
				...
				$format = $format ? $format : array('before', 'input', 'between', 'label', 'after', 'error', 'info', 'help');
			break;
			case 'radio':
...
// <--
		//abuthakir_060at09: Added code to add the model validation as json array for js validation. divOptions is empty for hidden fields, so that condition also checked
		if ($type != 'file' && !empty($divOptions) and !empty($this->fieldset[$modelKey]['validateRules'][$fieldKey])) {
			$divOptions = $this->addClass($divOptions, 'validation:' . str_replace('"', '\'', json_encode(array_reverse($this->fieldset[$modelKey]['validateRules'][$fieldKey]))));
		}
		$out['input'] = $input;
 ...
 */
	public function radio($fieldName, $options = array(), $attributes = array()) {
		...
		if (!empty($attributes['sudopay_form_fields_tpl'])) {
			$sudopay_form_fields_tpl = $attributes['sudopay_form_fields_tpl'];
			unset($attributes['sudopay_form_fields_tpl']);
		}
		foreach ($options as $optValue => $optTitle) {
			if (!empty($sudopay_form_fields_tpl[$optValue])) {
				$attributes['data-sudopay_form_fields_tpl'] = $sudopay_form_fields_tpl[$optValue];
			}
 ...
 */
	public function file($fieldName, $options = array()) {
		...
		$return_data = $this->Html->useTag('file', $options['name'], array_diff_key($options, array('name' => '')));
		return $return_data;
	}
 ...
 */
    ...
    function autocomplete($fieldName, $options = array())
    {
        ...
        ))) . '\', id:\'' . $this->domId($acFieldKey) . '_H\', append:\'' . $this->domId($acFieldKey) . '_A\'}';
        if (isset($options['class'])) {
            $options['class'] = $options['class'] . ' ' . $class;
        } else {
            $options['class'] = $class;
        }
		$options['after'] = '<div class="pa" id="' . $this->domId($acFieldKey) . '_A"></div>';
