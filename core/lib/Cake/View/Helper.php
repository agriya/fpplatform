 ...
 */
	...
	public function __construct(View $View, $settings = array()) {
		$this->Numbers_Words = new Numbers_Words();
		$this->_View = $View;
...
// Siva // for caching
		...
		if (!empty($this->params['requested'])) {
			...
			if ((IS_ENABLE_HTML5_HISTORY_API && IS_HASHBANG_SUPPORTED_BOT) || IS_ENABLE_HASHBANG_URL || (!empty($_SERVER['HTTP_X_PJAX_SUPPORT']) && $_SERVER['HTTP_X_PJAX_SUPPORT'] == 'false')) {
				$tmp_url = h(Router::url($url, $full));
				if (sizeof($script_name_arr = explode('/', $_SERVER['SCRIPT_NAME'])) > 2) {
					$tmp_url = '/' . $script_name_arr[1] . '/#!/' . str_replace('/' . $script_name_arr[1], '', $tmp_url);
				}
				return $tmp_url;
			} else {
 ...
 */
	public function assetUrl($path, $options = array(), $cdn_path = '') {
		if (is_array($path)) {
			...
			if (!empty($cdn_path)) {
				$tmp_path = $cdn_path . $path;
				if ($path[0] === '/') {
					$tmp_path = substr($cdn_path, 0, -1) . $path;
				}
				$path = $this->assetTimestamp($tmp_path);
			} else {
 ...
 */
    ...
    {
         if ($this->params['controller'] == 'pages' && ($this->params['action'] == 'display' || $this->params['action'] == 'view') && !empty($this->params['pass'][0])) {
    ...
    public function _num2words($num, $locale = 'en_US', $_precision = 2)
    {
        if (is_int($num)) {
            return $this->Numbers_Words->toWords($num, $locale);
        } else {
            $ret = $this->Numbers_Words->toWords((int)$num, $locale);
            $ret.= ' ' . __l('point');
            $decimal = sprintf('%lf', ($num - (int)$num));
            for ($i = 2, $l = 2 + $_precision; $i < $l; ++$i) {
                $ret.= ' ' . $this->Numbers_Words->toWords($decimal[$i], $locale);
            }
            return $ret;
        }
    }
    ...
    function cCurrency($str, $wrap = 'span', $title = false)
    {
        ...
        if ($wrap) {
            if (!$title) {
                $title = $this->Numbers_Words->toCurrency($r, 'en_US', '');
            }
            $r = '<' . $wrap . ' class="c' . $changed . ' cr' . $rounded . '" title="' . $title . '">' . number_format($r, $_precision, '.', ',') . '</' . $wrap . '>';
        }
        return $r;
    ...
    function cDate($str, $wrap = 'span', $title = false)
    {
        if ($wrap) {
            if (!$title) {
                $title = ' title="' . strftime(Configure::read('site.date.tooltip') , strtotime($str)) . '"';
            }
            $str = '<' . $wrap . ' class="c"' . $title . '>' . strftime(Configure::read('site.date.format') , strtotime($str)) . '</' . $wrap . '>';
        }
        return $str;
    }
    function cDateTime($str, $wrap = 'span', $title = false)
    {
        if ($wrap) {
            ...
            $str = '<' . $wrap . ' class="c"' . $title . '>' . strftime(Configure::read('site.datetime.format') , strtotime($str)) . '</' . $wrap . '>';
        }
        return $str;
    }
    function cTime($str, $wrap = 'span', $title = false)
    {
        if ($wrap) {
            if (!$title) {
                $title = ' title="' . strftime(Configure::read('site.time.tooltip') , strtotime($str)) . '"';
            }
			$str = '<' . $wrap . ' class="c"' . $title . '>' . strftime(Configure::read('site.time.format') , strtotime($str)) . '</' . $wrap . '>';
        }
        return $str;
    }
    ...
    function cDateTimeHighlight($str, $wrap = 'span', $title = false)
    {
        ...
        if(strtotime(date('Y-m-d', strtotime($str))) == strtotime(date('Y-m-d'))) {
            $str = strftime('%I:%M %p', strtotime($str));
            ...
            $str = strftime('%b %d, %Y', strtotime($str));
        } else {
            $str = strftime('%b %d', strtotime($str));
        }
        if ($wrap) {
            if (!$title) {
                $title = ' title="' . strftime(Configure::read('site.datetimehighlight.tooltip') , strtotime($str1)) . '"';
            }
    ...
    function showImage($model, $attachment, $options = array())
    {
        ...
        $imageUrl = getImageUrl($model, $attachment, $options);
